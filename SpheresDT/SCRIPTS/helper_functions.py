...
def slice_tif(na_tif=np.zeros((0)),ix_slice_z=None,ix_slice_time=None,
	...
	if len(na_tif.shape)==4 or (len(na_tif.shape)==5 and RGB):
		if ix_slice_z and ix_slice_time:
			na_tif_slice = na_tif[ix_slice_time[0]:ix_slice_time[1],ix_slice_z[0]:ix_slice_z[1],:,:]
		elif ix_slice_time:
			na_tif_slice = na_tif[ix_slice_time[0]:ix_slice_time[1],...]
		elif ix_slice_z:
			na_tif_slice = na_tif[:,ix_slice_z[0]:ix_slice_z[1],:,:]
		else:
			print('no select')
			na_tif_slice=na_tif
...
def convert_16bit_to_RGB(a_img_16bit,clip_not_scale=True,d_save_info=None):
	...
	a_4D_RGB = np.asarray([grey2rgb(i,alpha=None) for i in a_img_8bit])
