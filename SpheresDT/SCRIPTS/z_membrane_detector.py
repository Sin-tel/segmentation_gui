...
def detect_z_membrane(a_input,a_membrane_mask,a_membrane,a_exterior_mask,
	...
	# a_ddz_smoothedXY = gaussian(a_ddz, **d_gaussian_2)
	a_ddz_smoothedXY_overlay = gaussian(a_ddz_overlay, **d_gaussian_2)
	# a_ddz_smoothedXY_overlay = np.where(a_extended_mask==0,0,a_ddz_smoothedXY)
	...
	if use_masked_peak_image:
		print('using the masked image for peak detection..')
	...
	# step 6 : remove peaks in lower layers where signal deteriorates
	...
	#            extra metric : track S/N of every slice (use as switch)
	...
	if l_SN_ratio[ix_min] < SN_thresh:  # we use the first peak slice as a proxy for overall stack quality.  If S/N ratio of this slice drops below 2, we switch of z-membrane completely
		ix_max = ix_min
		print("z_membrane switched OFF for this stack, signal quality stack is to poor:")

	# #temp : 1 peak is not enough
	# if ix_max + 1 < a_bool_peak_img.shape[0] and np.sum(a_bool_peak_img[ix_max])<2:
	#     ix_max += 1
	...
	for ix_z, a_slice_i in enumerate(a_iter):
		...
		if np.sum(a_seed[ix_z])==0:  #no peaks or peaks with zero value
			...
			if 	peak_thresh_rel_slice>0:
				print("threshold_extension logic active :{0}".format(peak_thresh_rel_slice))
