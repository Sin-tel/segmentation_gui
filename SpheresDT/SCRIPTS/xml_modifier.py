...
restart_module = int(sys.argv[7]) if len(sys.argv)>7 else 0
...
if d_config['input_xml']:
	...
	for tab_i in switch_tab: 
		print(">>modifying PARAM state with tab{0} from {1}<<".format(tab_i,INPUT_CSV))
	...
	xml_element = root.findall('./RAM/script_folder')[0]
...
if sub_folder:
	(output_folder/sub_folder).mkdir(parents=True,exist_ok=True)
	print((output_folder/sub_folder)," has been created")
else:
	output_folder.mkdir(parents=True,exist_ok=True)
	print((output_folder," has been created. subfolder equals ",sub_folder))
